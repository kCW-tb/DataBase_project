...
def decrease_current_stock(n):
    stocks = db.stock_collection.find()  # 모든 데이터 가져오기
    data_to_update = None
    for idx, stock in enumerate(stocks):
        if idx == n-1:  # 0부터 시작하는 인덱스를 고려하여 n-1로 체크
...
while cap.isOpened():
    ...
    if current_time - last_update_time >= 70:  # 70초마다 업데이트
        print("재고 주문 전 현 상황")
        show_stock_data()
        update_stock_values()  # 주식 가격을 업데이트합니다.
        print("재고 주문 후 현 상황")
        show_stock_data()
        last_update_time = current_time  # 마지막 업데이트 시간을 갱신합니다.
    ...
    # 미디어파이프에 손 이미지를 전달하여 스켈레톤을 추출합니다.
